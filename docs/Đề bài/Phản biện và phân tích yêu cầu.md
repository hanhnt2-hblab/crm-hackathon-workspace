...
|---|---|---|
| **Mục 0** | Tầng chuẩn: từ vựng, enum, bảng số, 39 quyết định chốt `D1`…`D39`, trạng thái mọi `Q` và `F` | Trước khi viết dòng mã đầu tiên |
...
| **Mục 5** | Đối chiếu với CRM thật của HBLAB: tự trả lời `Q`, sửa `F`, thêm `F25`…`F38` | Khi cần con số đã có người trả giá |
...
## 0.3. Bảng quyết định chốt `D1`–`D39`
...
| `D39` | Mọi việc quan trọng có sinh log phải diễn ra **lại** trong ngày thi (viết kiểm thử, refactor, sinh tài liệu), vì thể lệ nói phần làm trước thời điểm thu log không được tính | chờ BTC | `Q22`, `F24` | không |
...
| `Q20` | có | `D26` | không |
| `Q21` | có | `D29` | không |
| `Q22` | **không** | `D39` là phương án phòng thân | **phải hỏi** — mốc tính log |

**Tổng: 2 câu chặn** (`Q15`, `Q22`) và **4 câu nên hỏi nhưng không chặn** (`Q6`, `Q10`, `Q12`,
`Q16`). Danh sách này thay cho mọi con số đếm khác trong tài liệu.

## 0.5. Trạng thái 38 mâu thuẫn `F1`–`F38`
...
| `F24` | Thể lệ tự bất nhất về mốc tính log | **không giải được** — chờ `Q22` | giảm thiểu bằng `D39` |
